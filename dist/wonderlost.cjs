...
class Tome {
  ...
  addHook(event, callback, overwrite = false) {
    if (!this.hooks.has(event) || overwrite) {
      ...
      consola__default.warn(`Hook for event "${event}" already exists.`);
    }
  }
  registerHooks() {
    this.hooks.forEach((callback, event) => {
      ...
      if (this.DEBUG)
        consola__default.info(`Registered hook for event: ${event}`);
    });
    return this;
  }
...
class Toasted extends Tome {
  ...
  static expandSidebarInstant(sidebar) {
    ...
    const sideB = new mote.El(sidebar).removeClass("collapsed").unset(["width", "height"]);
    ...
    new mote.El(icon).removeClass("fa-caret-left").addClass("fa-caret-right");
  ...
  static findTarget(card, event, messageID) {
    ...
    let closestMessageID = new mote.El(closestMessage).data(
      "messageId"
    );
    ...
    for (let vert = targetRect.top + 1; vert < targetRect.bottom; vert += dy) {
      ...
      for (let hor = targetRect.left + 1; hor < targetRect.right; hor += dx) {
        ...
        closestMessageID = new mote.El(closestMessage).data(
          "messageId"
        );
        if (target && closestMessageID === messageID)
          return { target, x, y };
  ...
  delegateEvent(n, ev) {
    const node = new mote.El(n);
    const card = new mote.El("#chat-log").element.querySelector(
  ...
  addMessage(node) {
    const div = new mote.El(`.${this.moduleName}`);
    ...
    all_js.TweenMax.from(node, 0.3, {
      height: 0,
      onComplete: () => {
        new mote.El(node).set({ height: "" });
...
class Wonderlost {
  constructor(DEBUG = false) {
    ...
    consola__default.info("Wonderlost | Initializing");
  ...
  initializeTomes() {
    this.tomes.forEach((tome, tomeName) => {
      ...
      if (this.DEBUG) {
        consola__default.info(`Wonderlost | Initialized ${tome.name}`);
      }
    });
  }
}
