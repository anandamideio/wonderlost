...
export abstract class Tome {
  ...
  constructor(
    ...
    this.stylesheets = pTome?.stylesheets ?? [];
    this.DEBUG = pTome?.DEBUG ?? false;
  }

  public addHook(event: HookableEvents, callback: HookEvent, overwrite = false) {
  ...
  public initializeStylesheets() {
    if (this.stylesheets.length === 0) return this;

    for (const stylesheet of this.stylesheets) {
      if (this.DEBUG) {
        consola.info(`Registering stylesheet @ ${stylesheet}`);
      }

      const cssPath =
        stylesheet.startsWith('/') || stylesheet.includes('://')
          ? stylesheet // Use as is if absolute or URL
          : `modules/wonderlost/submodules/${this.lowercaseName}/${stylesheet}`;

      // Check if already loaded
      if (document.querySelector(`link[href="${cssPath}"]`)) {
        if (this.DEBUG) {
          consola.info(`Stylesheet already loaded: ${cssPath}`);
        }
        return;
      }
