...
export class Limbo extends Tome {
	...
	private playerWaitingScreen:
		| ConfiguredStoredDocument<typeof Scene>
		| null
		| undefined;
						...
						if (notesControl) {
							...
							notesControl.tools.push({
								...
								onClick: () => {
									if (this.DEBUG) {
										consola.info(
											"Limbo | Button clicked, activating waiting room",
										);
									}
	...
	private async findOrCreateWaitingRoom(): Promise<void> {
		try {
			...
			const existingScene = game.scenes?.find(
				(scene) => scene.name === this.waitingRoomName,
			);

			if (existingScene) {
				if (this.DEBUG) {
					consola.info(
						`Limbo | Found existing waiting room scene "${this.waitingRoomName}"`,
						existingScene,
					);
				}
				...
				if (this.DEBUG) {
					consola.info(
						`Limbo | Creating new waiting room scene "${this.waitingRoomName}"`,
					);
				}
				...
				if (this.DEBUG && this.playerWaitingScreen) {
					consola.success(
						"Limbo | Created new waiting room scene",
						this.playerWaitingScreen,
					);
				}
			}
		} catch (error) {
	...
	public async activateWaitingRoom() {
		...
		if (!this.playerWaitingScreen) {
			consola.warn(
				"Limbo | Waiting room not found, attempting to create one...",
			);
			await this.findOrCreateWaitingRoom();
